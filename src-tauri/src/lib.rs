...
fn emit_status(app: &AppHandle, stage: &str, message: &str, detail: Option<String>) {
    if HEADLESS.load(Ordering::Relaxed) {
        println!("{} [{stage}] {message} {}", Local::now().format("%H:%M:%S"), detail.as_deref().unwrap_or(""));
    }
    let _ = app.emit(
        "daily-status",
        Status {
...
async fn print_edition(app: AppHandle, target: String) -> Result<String, String> {
    ...
    std::process::Command::new(opener)
        .arg(&pdf)
        .spawn()
        .map_err(|e| format!("Made the PDF but couldn't open it ({e}): {}", pdf.display()))?;
...
async fn build_edition(app: &AppHandle) -> Result<Edition, String> {
    ...
    let claude_bin = paths::find_bin("claude", &settings.claude_bin).await.ok_or_else(|| {
        "Can't find the `claude` CLI. If it works in your terminal, run `which claude` there and put that path in settings.json as claudeBin.".to_string()
    })?;
        ...
        match &grok_bin {
            ...
            None => grok::GrokOutcome {
                ...
                note: Some(if settings.use_grok {
                    "X wire: Grok CLI not found. Install it (curl -fsSL https://x.ai/cli/install.sh | bash), run `grok login`, and the X Wire section fills in. Claude covered X as best it could.".to_string()
                } else {
    ...
    // macOS blocks plain-http image loads inside apps; nearly every image
...
async fn refresh_headless(app: AppHandle) -> i32 {
    ...
    // launchd also fires a missed job on wake. If an edition was printed in the
    ...
    if let Some(e) = store::load_edition(&app, &today) {
        let fresh = chrono::DateTime::parse_from_rfc3339(&e.generated_at)
            ...
            .unwrap_or(false);
        if fresh {
            println!("Today's edition is less than 3 hours old. Not reprinting it to screen.");
    ...
    let Some(_lock) = store::RefreshLock::acquire(&app) else {
        println!("Another refresh is already running. Leaving it to finish.");
        return 0;
    };
    ...
    for attempt in 0..18 {
        ...
        for probe in ["https://api.anthropic.com/", "https://www.apple.com/"] {
        ...
        if attempt == 0 {
            println!("No network yet, waiting...");
        }
        tokio::time::sleep(std::time::Duration::from_secs(10)).await;
    }
    if !online {
        println!("Still offline after 3 minutes. Giving up; the app will print an edition when it's opened.");
        return 1;
    }

    match build_edition(&app).await {
        Ok(e) => {
            println!("Edition ready: {} stories in {}s.", e.cards.len(), e.stats.duration_secs);
            if let Some(msg) = print::morning_print(&app, &e).await {
                println!("Paper: {msg}");
            }
            0
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            // An install from the "Richard's Daily" days keeps its data and schedule.
            store::migrate_legacy(app.handle());
            if headless {
                // No window, no Dock icon: do the job and leave.
                #[cfg(target_os = "macos")]
                ...
                tauri::WebviewWindowBuilder::from_config(app.handle(), &config)?.title(title).build()?;
                schedule::migrate_legacy(app.handle());
            }
            Ok(())
        })
        .on_window_event(|window, event| {
            // One-window app: closing the front page quits (and takes any open
            // reader windows with it) instead of lingering windowless in the Dock.
