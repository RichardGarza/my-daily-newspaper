...
//! (macOS: ~/Library/Application Support/com.mydailynewspaper.app).
...
/// This app used to be "Richard's Daily" with its own data folder. Bring an
/// existing install along: interests, settings, editions. Runs once.
pub fn migrate_legacy(app: &AppHandle) {
    let Ok(new_dir) = data_dir(app) else { return };
    if new_dir.join("interests.json").exists() || new_dir.join("settings.json").exists() {
        return;
    }
    let Some(old_dir) = new_dir.parent().map(|p| p.join("com.richardgarza.daily")) else { return };
    if !old_dir.join("interests.json").exists() {
        return;
    }
    for name in ["interests.json", "settings.json", "channel-cache.json"] {
        let _ = fs::copy(old_dir.join(name), new_dir.join(name));
    }
    if let Ok(rd) = fs::read_dir(old_dir.join("editions")) {
        for e in rd.flatten() {
            let _ = fs::copy(e.path(), new_dir.join("editions").join(e.file_name()));
        }
    }
    // Someone with a customised list doesn't need the welcome tour.
    let _ = update_settings(app, |s| s.onboarded = true);
}
